...
description: 有關"陋習"一詞,首先它并不是一個中性的,描述性的詞匯[^1],而是帶有明顯評價負載的規范性詞匯,以及將描述與評價壓縮在一起的語言形式,其中有包含著兩個先驗(劃去)典型的印象感知層面[^2],對行爲性經驗的刻畫(如習慣存在.頻率與後果).以及對於行爲的價值判定(涉嫌是否應該被否定,限制或改造). 這裏我借用事實判…
locale: zh-cn
